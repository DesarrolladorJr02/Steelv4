...
// TODO PARA REGISTRAR ASISTENCIA DESDE EL QR EN STEELPRO
const mostAltaconqr = require('./checadorhuella/asistenciach/mostAltaconqr');
...
app.get("/buscarPersonalValor", async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(404).json({
            mensaje: 'Hubo un error al procesar la solicitud (buscarPersonalValor).'
        });
    }
})
app.put("/registrarhuella", async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(404).json({
            mensaje: 'Hubo un error al procesar la solicitud (registrarhuella).'
        });
    }
})
...
app.post("/registrarEntrada", async (req, res) => {
    ...
    try {
        ...
        const RN_Verificar_si_Personal_YaMarco_su_Falta = await verificarfalta(idalta);
        const Verificarfalta = Object.values(RN_Verificar_si_Personal_YaMarco_su_Falta[0])[0];
        //console.log("Verificarfalta ", Verificarfalta.Id_asis);
        if (dia === "Saturday " || dia === "Sunday") {
            ...
            if (horaactualcomparar <= hoEntradaSD) {
                if (!respuestaasistencias) {
                    if (Verificarfalta) {
                        ...
                        respuesta = await actualizarEntrada(Verificarfalta.Id_asis, horaactual, xJustificacion, xEstadoAsis);
                    ...
                    const xJustificacion = "NA";
                    const xEstadoAsis = "Entrada";
                    const idasistencia = (Verificarfalta) ? Verificarfalta.Id_asis : respuestaasistencias.Id_asis;
                    respuesta = await actualizarEntrada(idasistencia, horaactual, xJustificacion, xEstadoAsis);
                    io.emit('escuchandoAsistencia', "Actualizartablaasis");
                }
        ...
        console.log(error);
        res.status(404).json({
            mensaje: 'Hubo un error al procesar la solicitud (registrarEntradaconfalta).'
        });
    }

})
//Copia del original (registrarEntrada)
/* app.post("/registrarEntrada", async (req, res) => {
    //idalta, nombredia, hoingreso
    try {
        const fechaactual = moment().format("YYYY-MM-DD");
...
//Se debe agregar a produccion para actualizar la informacion de los usuarios que se marcaron como falta
app.put("/registrarEntradaconfalta", async (req, res) => {
    try {
        const fechaactual = moment().format("YYYY-MM-DD");
        const horaactual = moment().format("HH:mm:ss");
        const horaactualcomparar = moment().format("HH:mm");
...
app.get("/selectfechas", async (req, res) => {
    try {
        ...
        if (req.query.id != "NA" && req.query.fechainicio && req.query.dias != "NA") {
            //console.log(req.query);
...
/* TODO PARA REGISTRAR ASISTENCIA DESDE EL QR EN STEELPRO */
//const mostAltaconqr = require('./checadorhuella/asistenciach/mostAltaconqr');
app.get('/Altaconqr', async (req, res) => {
    try {
        const idcheck = req.query.id;
        //console.log("idcheck ", idcheck);
        const dia = moment().format("dddd");
        const otrorespuesta = await mostAltaconqr(idcheck);
        let respuesta = {};
        if (dia === "Saturday " || dia === "Sunday") {
            respuesta = otrorespuesta.map((datos) => {
                return {
                    Area: datos.Area,
                    Estatus: datos.Estatus,
                    HEntrada: datos.HoEntradaSD,
                    HSalida: datos.HoSalidaSD,
                    NombreCompleto: datos.NombreCompleto,
                    Puesto: datos.Puesto,
            ...
            respuesta = otrorespuesta.map((datos) => {
                return {
                    Area: datos.Area,
                    Estatus: datos.Estatus,
                    HEntrada: datos.HoEntrada,
                    HSalida: datos.HoSalida,
                    NombreCompleto: datos.NombreCompleto,
                    Puesto: datos.Puesto,
                    Sucursal: datos.Sucursal,
                    idAlta: datos.idAlta,
...
app.post('/registrarentradaqr', async (req, res) => {
    try {
        const fechaactual = moment().format("YYYY-MM-DD");
        const horaactual = moment().format("HH:mm:ss");
        const horaactualcomparar = moment().format("HH:mm");

        const idalta = req.body.idAlta;
        const horario = await listarHorario(idalta);
        const dia = moment().format("dddd");
        let respuesta = [];
        const respuestaasistencias1 = await mostListaasistencia();
        ...
        const RN_Verificar_si_Personal_YaMarco_su_Asistencia_api = await Verificarasistencia(idalta);
        const verificarasistencia = Object.values(RN_Verificar_si_Personal_YaMarco_su_Asistencia_api[0])[0];
        //console.log("verificarasistencia ", verificarasistencia);
        if (verificarasistencia === 0) {
            const RN_Verificar_si_Personal_YaMarco_su_Entrada_api = await verificarentrada(idalta);
            const Verificarentrada = Object.values(RN_Verificar_si_Personal_YaMarco_su_Entrada_api[0])[0];
            if (Verificarentrada === 0) {
                const RN_Verificar_si_Personal_YaMarco_su_Entrada_Ayer_api = await verificarentradaAyer(idalta);
                const Verificarentradaayer = Object.values(RN_Verificar_si_Personal_YaMarco_su_Entrada_Ayer_api[0])[0];
                if (Verificarentradaayer === 0) {
                    const RN_Verificar_si_Personal_YaMarco_su_Falta = await verificarfalta(idalta);
                    const Verificarfalta = Object.values(RN_Verificar_si_Personal_YaMarco_su_Falta[0])[0];
                    //console.log("Verificarfalta ", Verificarfalta.Id_asis);
                    if (dia === "Saturday " || dia === "Sunday") {
                    ...
                    res.status(400).json({
                        mensaje: 'No cerraste la asistencia de ayer!'
                    });
                }
            } else {
                res.status(400).json({
                    mensaje: 'Tu entrada ya fue registrada, no insistir!'
                });
            }
        } else {
            res.status(400).json({
                mensaje: 'Ya marcaste tu asistencia, no insistir!'
            });
        }
        ...
        console.log(error);
        res.status(404).json({
            mensaje: 'Hubo un error al procesar la solicitud (registrarentradaqr).'
        });
    }
})

app.put('/registrarsalidaqr', async (req, res) => {
    try {
        const dato = req.body.idAlta;
        console.log("dato ", dato);
        const RN_Verificar_si_Personal_YaMarco_su_Asistencia_api = await Verificarasistencia(dato);
        const verificarasistencia = Object.values(RN_Verificar_si_Personal_YaMarco_su_Asistencia_api[0])[0];
        if (verificarasistencia === 0) {
            const RN_Verificar_si_Personal_YaMarco_su_Entrada_api = await verificarentrada(dato);
            const Verificarentrada = Object.values(RN_Verificar_si_Personal_YaMarco_su_Entrada_api[0])[0];
            if (Verificarentrada > 0) {
                const dataasis = await listarasistenciareciencreada(dato);
                const idasis = dataasis[0].Id_asis;
                //console.log("idasis ", req.body);
                const horaactual = moment().format("HH:mm:ss");
                const fechactual = moment().format("YYYY-MM-DD");
                const actual = moment(`${fechactual} ${horaactual}`, 'YYYY-MM-DD HH:mm:ss');
                ...
                const RN_Verificar_si_Personal_YaMarco_su_Entrada_Ayer_api = await verificarentradaAyer(dato);
                const VerificarentradaAyer = Object.values(RN_Verificar_si_Personal_YaMarco_su_Entrada_Ayer_api[0])[0];
                if (VerificarentradaAyer > 0) {
                    const dataasis = await listarasistenciareciencreada(dato);
                    const idasis = dataasis[0].Id_asis;
                    ...
                    res.status(400).json({
                        mensaje: 'No haz registrado tu entrada'
                    });
                }
        ...
        console.log(error);
        res.status(404).json({
            mensaje: 'Hubo un error al procesar la solicitud (registrarsalidaqr).'
        });
    }
